...
for e in ens:
    ...
    tmp = xr.open_dataset(folder_mod[e]+fmod[e])
    # randchoice.append(e)
    for t in range(len(tmp.time_lag)):
        ds[f"{e}_tlag{t:02d}"] = tmp.isel(time_lag=t)
        randchoice.append(f"{e}_tlag{t:02d}")
...
chosen_ens = np.random.choice(randchoice)
print(f"Chosen member: {chosen_ens}")
ds_ens_out = ds[chosen_ens]
...
ds_ens_out.to_netcdf(f"{folder_out}{wave}_tlag_da_vorticity_budget_{init}_ensrand_remap_padded.nc", encoding=encoding)
