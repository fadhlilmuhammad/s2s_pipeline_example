...
folder_time = f'/g/data/v46/fm6730/dataout/lifecycle/rlut/{wave}/{mode}/'
file_time = f'ens_tlag_choices_lon{ctrl_lon}_{mode}_{wave}_{season}_rlut_ensrand_leaddays.csv'
time_active_df = pd.read_csv(os.path.join(folder_time, file_time))

time_active_str = time_active_df['init'].astype(str).tolist()
...
for init in init_sets:
    ...
    e = time_active_df.loc[time_active_df['init'] == int(init), 'ens'].values[0]
    tlag = time_active_df.loc[time_active_df['init'] == int(init), 'tlag'].values[0]

    print(f'Processing member randomly chosen: {e} at time lag {tlag}')
    ...
    ds_init_mod = xr.open_dataset(os.path.join(folder_mod, file_mod))
    ds_mod = ds_init_mod.isel(time_lag=tlag)
    ...
    ds_mod_sel = ds_mod.sel(lat=slice(15, -15),time=slice(init,None))
    # da_obs_sel = da_obs.sel(lat=slice(15, -15),time=slice(init,None))
    ds_dia_sel = ds_dia.sel(lat=slice(15, -15),time=slice(init,None))
    ...
    # %%
    weights_mod = np.cos(np.deg2rad(ds_mod_sel['lat']))
    # weights_obs = np.cos(np.deg2rad(da_obs_sel['lat']))
    weights_dia = np.cos(np.deg2rad(ds_dia_sel['lat']))

    print(f'Length of ds_mod_sel: {len(ds_mod_sel["time"])}')

    weighted_mod_mean = ds_mod_sel.weighted(weights_mod).mean(dim='lat')
    # weighted_obs_mean = da_obs_sel.weighted(weights_obs).mean(dim='lat')
    weighted_dia_mean = ds_dia_sel.weighted(weights_dia).mean(dim='lat')
...
outfile = os.path.join(folderout, f'hovmuller_lon{ctrl_lon}_{mode}_{wave}_{season}_vorticity_budget_ensrand_leaddays.nc')
